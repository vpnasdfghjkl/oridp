...
from kuavo_msgs.msg import lejuClawState, sensorsData # type: ignore
...
class ObsBuffer:
    def __init__(self, img_buffer_size: int = 30, robot_state_buffer_size: int = 100, obs_key_map: Optional[Dict[str, Dict[str, str]]] = None) -> None:
        ...
        # 直接在这里订阅话题并指定回调函数
        rospy.Subscriber("/camera/color/image_raw", CompressedImage, self.Image_callback)
        rospy.Subscriber("/sensors_data_raw", sensorsData, self.sensorsData_callback)
        rospy.Subscriber("/leju_claw_state", lejuClawState, self.lejuClawState_callback)
    
    def compressedImage_callback(self, msg: CompressedImage):
        key = "img01"
        ...
        # print(self.obs_buffer_data[key]['timestamp'][-1], time.time())
    ...
    def sensorsData_callback(self, msg: sensorsData):
        key = "state_joint"
        joint = msg.joint_data.joint_q[12:26]
        self.obs_buffer_data[key]["data"].append(joint)
        self.obs_buffer_data[key]["timestamp"].append(msg.header.stamp.to_sec())
    
    def lejuClawState_callback(self, msg: lejuClawState):
        key = "state_gripper"
    ...
    def get_lastest_k_img(self, k: int) -> Dict[int, Dict[str, np.ndarray]]:
        ...
        print(f"{out[0]['timestamp'][-1]:.10f}, {out[0]['timestamp'][-2]:.10f}")
        return out
...
class KuavoEnv:
    ...
    def get_obs(self, just_img: bool=False) -> dict:
        ...
        # print(camera_obs_timestamps['img01'], robot_obs_timestamps['ROBOT_state_joint'],robot_obs_timestamps['ROBOT_state_gripper'],  )
        # print('check')
        # ==========================================
        ...
        if not just_img:
            ...
            robot_final_obs["agent_pos"] = np.concatenate((robot_obs["ROBOT_state_joint"][:,7:], robot_obs["ROBOT_state_gripper"][:,1:]), axis=-1)
...
if __name__ == "__main__":
    try:
        ...
        while 1:
            cur_obs, camera_obs, camera_obs_timestamps, robot_obs, robot_obs_timestamps = env.get_obs()  
            print('here')
            # time.sleep(5)
            # print(f"{camera_obs_timestamps['img01'][0]:.10f}, {camera_obs_timestamps['img01'][1]:.10f}, \n"
            #     f"{robot_obs_timestamps['ROBOT_state_joint'][0]:.10f}, {robot_obs_timestamps['ROBOT_state_joint'][1]:.10f}, \n"
            #     f"{robot_obs_timestamps['ROBOT_state_gripper'][0]:.10f}, {robot_obs_timestamps['ROBOT_state_gripper'][1]:.10f}\n")
            # robot_obs_timestamps['ROBOT_state_gripper']
            # print('')
            # print('c')
