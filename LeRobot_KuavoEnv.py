...
from kuavo_msgs.msg import lejuClawState, sensorsData, robotHandPosition    # type: ignore
...
class ObsBuffer:
    def __init__(self, img_buffer_size: int = 30, robot_state_buffer_size: int = 100, obs_key_map: Optional[Dict[str, Dict[str, str]]] = None) -> None:
        ...
        self.callback_key_map = {
            ...
            # lejuClawState: self.lejuClawState_callback,
    ...
    def compressedImage_callback(self, msg: CompressedImage, key: str):
        ...
        resized_img = cv2.resize(cv_img, (480, 640))
    ...
    def robotHandPosition_callback(self, msg: robotHandPosition, key: str):
        ...
        self.obs_buffer_data[key]["data"].append(hand_data)
        self.obs_buffer_data[key]["timestamp"].append(msg.header.stamp.to_sec())
    ...
    def sensorsData_callback(self, msg: sensorsData, key: str):
        # Float64Array ()
        joint = [float(i) for i in msg.joint_data.joint_q[:]]
        self.obs_buffer_data[key]["data"].append(joint)
        self.obs_buffer_data[key]["timestamp"].append(msg.header.stamp.to_sec())
    
    def lejuClawState_callback(self, msg: lejuClawState, key: str):
        # Float64Array ()
...
class TargetPublisher:
    ...
    def control_hand(self, left_hand_position: List[float], right_hand_position: List[float]):
        ...
        try:
            ...
            return False
    # def control_claw(self, position):
...
class KuavoEnv:
    ...
    def get_obs(self, just_img: bool=False) -> dict:
        ...
        k_image = min(self.img_buffer_size, math.ceil((self.n_obs_steps + 2) * (self.video_capture_fps / self.frequency)))
        ...
        print("obs_align_timestamps ", [f"{t:.10f}" for t in obs_align_timestamps])
        camera_obs = dict()
        camera_obs_timestamps = dict()
        for camera_idx, value in self.last_realsense_data.items():
            this_timestamps = value["timestamp"]
            print(camera_idx, [f"{t:.10f}" for t in this_timestamps])
            this_idxs = list()
        ...
        if not just_img:
            ...
            robot_final_obs["agent_pos"] = np.concatenate((robot_obs["ROBOT_state_joint"][:,12:19], robot_obs["ROBOT_state_gripper"][:,:1]), axis=-1)
    ...
    def exec_actions(
        ...
        new_actions = actions   #(8, 26)
        # with open("cur_state.txt", "a") as f:
        #     f.write(str(cur_state) + "\n")
        for i in range(len(new_actions)):
            left_arm_joint = new_actions[i, :7]
            left_hand_joint = new_actions[i, 7:13]
            right_arm_joint = new_actions[i, 13:20]
            right_hand_joint = new_actions[i, 20:26]
            # print left_arm_joint, left_hand_joint, right_arm_joint, right_hand_joint
            print('=============one action Reference results====================')
            print('left_arm_joint:',left_arm_joint)
            print('left_hand_joint:',left_hand_joint)
            print('right_arm_joint:',right_arm_joint)
            print('right_hand_joint:',right_hand_joint)
            print('=============one action Reference results====================')
            
            #####################
            ### control arm joint
            #####################
            only_left_control = True
            if only_left_control:
                right_arm_joint = [0] * 7
            print('***************actually pub command***************')
            print('left_arm_joint:',left_arm_joint)
            print('right_arm_joint:',right_arm_joint)
            
            arm_joint = np.concatenate((left_arm_joint, right_arm_joint))
            self.target_publisher.publish_target_joint(arm_joint)
            ...
            if eef_type == "leju_claw":
                ...
                req.data.position = [90, new_actions[i, -1]]
                ...
                only_left_control = True
                if only_left_control:
                    right_hand_joint = [0] * 6
                    if new_actions[i, -1] > 10:
                        left_hand_joint = [100] * 6
                    else:
                        left_hand_joint = [0, 100, 0, 0, 0, 0]
                    # gather_than_threshold = left_hand_joint[left_hand_joint > 10]
                    # if gather_than_threshold.size > 0:
                    #     left_hand_joint = [100] * 6
                    # else:
                    #     left_hand_joint = [0, 100, 0, 0, 0, 0]
                print('left_hand_joint:',[max(0, int(angle)) for angle in left_hand_joint])
                print('right_hand_joint:', [max(0, int(angle)) for angle in right_hand_joint])
                print('***************actually pub command***************')
                msg.left_hand_position =  [max(0, int(angle)) for angle in left_hand_joint] # 左手位置
                msg.right_hand_position =  [max(0, int(angle)) for angle in right_hand_joint]  # 右手位置
                msg.header.stamp = rospy.Time.now() 
                pub.publish(msg)
            else:
                RuntimeError("Unknown eef type")
            time.sleep(0.25)
...
if __name__ == "__main__":
    try:
        ...
        while 1:
            cur_obs, camera_obs, camera_obs_timestamps, robot_obs, robot_obs_timestamps = env.get_obs(just_img=True)
            print('##obs timestamps################################################################')
